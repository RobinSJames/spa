    ...
    <div class="flex flex-col sm:flex-row p-1/12">
      <div class="w-full sm:w-1/2 pl-4">
        <div>
          <img
            :src="
              'https://mockspa-api.herokuapp.com/' + singlepackage.packageImage
            "
            alt=""
          />
        </div>
      ...
      <div class="w-full sm:w-1/2 px-4">
        ...
        <p v-if="singlepackage.availability" class="pb-4">
          {{ singlepackage.availability }}
        </p>
        <p class="pb-4">{{ singlepackage.excerpt }}</p>
        <p class="text-xl md:text-2xl text-dark-gray font-bold pb-4">
          {{ singlepackage.description }}
        </p>
        <p class="pb-4">{{ singlepackage.additionalDescription }}</p>
        ...
        <AppButton
          ...
          @clicked="addToCart(singlepackage._id, quantity)"
        />
      </div>
    </div>
...
export default {
  ...
  computed: {
    singlepackage() {
      return this.$store.state.packages.single
    },
    priceByQuantity() {
      return this.singlepackage.cost * this.quantity
    },
    // favourites() {
